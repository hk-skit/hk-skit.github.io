...
// Aspect ratios per column: outer cols are square, middle col mixes portrait + landscape
const colAspects = [
            ...
            <div key={colIdx} className="flex flex-col gap-2">
              {col.map((photo, idx) => (
                <BlurFade key={photo.src} delay={BLUR_FADE_DELAY * 14 + (colIdx * col.length + idx) * 0.05}>
                  <div className={`relative w-full overflow-hidden rounded-xl ${colAspects[colIdx][idx] ?? "aspect-square"}`}>
                    <img
                      src={photo.src}
                      alt=""
                      aria-hidden="true"
